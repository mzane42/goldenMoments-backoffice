...
export default function AdminExperiences() {
  ...
  // Batch mutations
  const batchUpdateStatusMutation = trpc.admin.experiences.batchUpdateStatus.useMutation({
    onSuccess: (result) => {
      if (result.failed === 0) {
  ...
  const batchDeleteMutation = trpc.admin.experiences.batchDelete.useMutation({
    onSuccess: (result) => {
      if (result.failed === 0) {
